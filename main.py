...
def layers(
    ...
    x = tf.add(x, vgg_layer3_out, name='skip_2')
...
def optimize(
    ...
    train_op = \
        tf.train.AdamOptimizer(learning_rate=learning_rate).minimize(cross_entropy_loss)
...
def train_nn(
    ...
    for epoch in range(epochs):
        ...
        for (images, labels) in get_batches_fn(batch_size):
            ...
            batch_num += 1
            print 'Epoch {:>2}, step: {}, loss: {}  '.format(epoch + 1,
                    batch_num, loss)
...
def run():
    ...
    with tf.Session() as sess:
        ...
        get_batches_fn = \
            helper.gen_batch_function(os.path.join(data_dir,
        ...
        (input_image, keep_prob, vgg_layer3, vgg_layer4, vgg_layer7) = \
            load_vgg(sess, vgg_path)
        nn_last_layer = layers(vgg_layer3, vgg_layer4, vgg_layer7,
                               num_classes)
        (logits, optimizer, cross_entropy_loss) = \
